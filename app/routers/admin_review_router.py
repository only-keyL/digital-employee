"""后台审核 REST API（阶段 5）。生产环境必须接鉴权，本阶段为内部 Mock 管理入口。"""
...
def approve_knowledge(knowledge_id: int, payload: KnowledgeReviewActionRequest, db: Session = Depends(get_db)):
    """审核通过 pending 知识：启用并入队 vector_sync_task，不阻塞等待 Qdrant。"""
    try:
        req = payload.model_copy(update={"action": "approve"})
        data = AdminReviewService(db).approve_knowledge(knowledge_id, req)
...
def reject_knowledge(knowledge_id: int, payload: KnowledgeReviewActionRequest, db: Session = Depends(get_db)):
    """审核拒绝 pending 知识：必须填写 audit_remark，不创建 vector_sync_task。"""
    try:
        req = payload.model_copy(update={"action": "reject"})
        data = AdminReviewService(db).reject_knowledge(knowledge_id, req)
...
def mark_contribution_reviewed(
    ...
    try:
        data = AdminReviewService(db).mark_contribution_reviewed(contribution_id, payload)
